...
void read_floppy(void *buffer, int lba, int sector_count) {
  ...
  dma::prepare_transfer(dma::FLOPPY_CHANNEL, buffer, transfer_size,
                        dma::mode::read);
  ...
  uint8_t result[7];
  issue_command_with_result(COMMAND_READ | MFM | MT, &result, head << 2,
                            cylinder, head, sector, 2, end_of_track, 0x1b,
                            0xff);
