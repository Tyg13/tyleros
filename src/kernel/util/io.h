...
__attribute__((__always_inline__)) static inline void wait() {
  ...
  asm volatile __inline__("outb %%al, $0x80" ::"a"(0) : "memory");
}

__attribute__((__always_inline__)) static inline void out(uint16_t port,
                                                          uint8_t val) {
  asm volatile __inline__("outb %0, %1" : : "a"(val), "Nd"(port) : "memory");
}

__attribute__((__always_inline__)) static inline uint8_t in(uint16_t port) {
  uint8_t ret;
